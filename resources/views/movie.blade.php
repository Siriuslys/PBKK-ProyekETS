    ...
    <div class="max-w-7xl mx-auto py-12">
        <div class="bg-white shadow rounded-lg p-6">
            ...
            <div class="flex flex-col md:flex-row mt-4"> {{-- Flex container for video and details --}}
                <div class="flex-[4_4_0%] mt-0"> {{-- Video container --}}
                    @if($movie->trailer)
                        <div class="w-full h-full">
                            ...
                            @if($videoId)
                                <iframe class="w-full h-[600px] mt-4" src="https://www.youtube.com/embed/{{ $videoId }}" frameborder="0" allowfullscreen></iframe>
                            @else
                ...
                <div class="flex-[1_1_0%] ml-6 mt-4"> {{-- Movie details container --}}
            ...
            <div class="mt-8">
                ...
                <h4 class="text-xl mt-6 font-bold">{{ $userReview ? 'Update Your Review' : 'Add a Review' }}</h4>
                <form action="{{ $userReview ? route('reviews.update', $userReview->id) : route('reviews.store', $movie->id) }}" method="POST" class="mt-4">
                    @csrf
                    @if($userReview) @method('PUT') @endif {{-- Gunakan metode PUT untuk update --}}
                    <div>
                        <textarea name="message" rows="4" class="w-full p-2 border border-gray-300 rounded" placeholder="Write your review..." required>{{ $userReview->message ?? '' }}</textarea>
                    </div>
                    <div class="mt-2">
                    ...
                    <button type="submit" class="mt-2 bg-blue-500 text-white px-4 py-2 rounded">{{ $userReview ? 'Update Review' : 'Submit Review' }}</button>
                </form>
                        ...
                        @foreach($reviews as $review)
                            <div class="border-b pb-2 mt-2">
                                ...
                                <div class="flex items-center mt-1">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <svg class="w-6 h-6 {{ $i <= $review->rating ? 'text-yellow-500' : 'text-gray-400' }}" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                                ...
                                @if(auth()->id() === $review->user_id) <!-- Check if the review belongs to the logged-in user -->
                                    <form action="{{ route('reviews.delete', $review->id) }}" method="POST" class="inline-block mt-2">
                                        @csrf
                                        @method('DELETE')
